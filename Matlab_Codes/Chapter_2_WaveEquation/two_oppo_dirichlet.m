...
t_postscript = 6.28;
postscript_filename = sprintf('pic_two_dirichlet_oppo_FD.eps');
...
% left wave
mask1 = x_sin <= period1;
u0_sin(mask1)  = sin(k1 * x_sin(mask1));
ut0_sin(mask1) = -c * k1 * cos(k1 * x_sin(mask1));

% right wave
mask2 = x_sin >= (I - period2);
u0_sin(mask2)  = u0_sin(mask2)  + sin(k2 * x_sin(mask2));
ut0_sin(mask2) = ut0_sin(mask2) + (+c * k2 * cos(k2 * x_sin(mask2)));
...
mask1_cheb = x_cheb_int <= period1;
mask2_cheb = x_cheb_int >= (I - period2);

u0_cdm(mask1_cheb)  = sin(k1 * x_cheb_int(mask1_cheb));
ut0_cdm(mask1_cheb) = -c*k1*cos(k1 * x_cheb_int(mask1_cheb));

u0_cdm(mask2_cheb)  = u0_cdm(mask2_cheb)  + sin(k2 * x_cheb_int(mask2_cheb));
ut0_cdm(mask2_cheb) = ut0_cdm(mask2_cheb) + c*k2*cos(k2 * x_cheb_int(mask2_cheb));
...
mask1_exact = x_exact <= period1;
mask2_exact = x_exact >= (I - period2);

u0_exact(mask1_exact)  = sin(k1 * x_exact(mask1_exact));
ut0_exact(mask1_exact) = -c*k1*cos(k1 * x_exact(mask1_exact));

u0_exact(mask2_exact)  = u0_exact(mask2_exact)  + sin(k2 * x_exact(mask2_exact));
ut0_exact(mask2_exact) = ut0_exact(mask2_exact) + c*k2*cos(k2 * x_exact(mask2_exact));
...
%% Video
video_filename = fullfile('videos', 'two_opposite_dirichlet.avi');
...
for n = 1:n_frames
    ...
    plot(x_exact, u_exact,'k--', 'LineWidth', 2.5, 'DisplayName', 'Exact (reference)');
