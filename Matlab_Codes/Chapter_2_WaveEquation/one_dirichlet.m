...
t_postscript = 12.56;
postscript_filename = sprintf('pic_one_dirichlet_FD.eps');
...
u0_sin(mask)  = sin(k * 2 * x_sin(mask));
ut0_sin(mask) = -c * (k*2) * cos(k * 2 * x_sin(mask));
...
mask_cheb = x_cheb_int <= pi/2;

u0_cdm(mask_cheb)  = sin(k * 2 * x_cheb_int(mask_cheb));
ut0_cdm(mask_cheb) = -c * (k*2) * cos(k * 2 * x_cheb_int(mask_cheb));
...
mask_exact = x_exact <= pi/2;

u0_exact(mask_exact)  = sin(k * 2 * x_exact(mask_exact));
ut0_exact(mask_exact) = -c * (k*2) * cos(k * 2 * x_exact(mask_exact));
...
%% Video
video_filename = fullfile('videos', 'one_dirichlet.avi');
