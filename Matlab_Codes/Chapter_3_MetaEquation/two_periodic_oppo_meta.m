...
function u_mode = exact_mode(t, x, k, amp, mu1, mu2, k1, k2, vel_sign)
    kappa = k;
    A_op = -kappa^2;
    ...
    C_sin = V_mat \ [1; 0];
    B_cos = (V_mat \ [vel_sign * (-kappa); 0]) ./ omegas;

    u_sin = C_sin(1)*V_mat(1,1)*cos(omegas(1)*t) + C_sin(2)*V_mat(1,2)*cos(omegas(2)*t);
    u_cos = B_cos(1)*V_mat(1,1)*sin(omegas(1)*t) + B_cos(2)*V_mat(1,2)*sin(omegas(2)*t);

    u_mode = amp * (u_sin .* sin(kappa * x) + u_cos .* cos(kappa * x));
end

u_exact_fun = @(t, x) exact_mode(t, x, k_wave1, amp1, mu1, mu2, k1, k2, +1) + exact_mode(t, x, k_wave2, amp2, mu1, mu2, k1, k2, -1);
...
print('-depsc2', '-r300', 'two_periodic_oppo_meta_DFT.eps');


video_filename = fullfile('videos', 'two_periodic_oppo_meta.avi');
...
for i = 1:length(tspan)
    ...
    clf; hold on;
    %plot(x, u_fd_exact(t),   'b-', 'LineWidth',2.5, 'DisplayName','FD');
    %plot(x, u_num_exact(t),  'r-', 'LineWidth',2.5, 'DisplayName','Numerov ');
    %plot(x, u_dft_exact(t),  'm-', 'LineWidth',2.5, 'DisplayName','DFT ');
    plot(x, u_exact_fun(t, x), 'k--', 'LineWidth',2.5, 'DisplayName','Exact');

    xlabel('x');
    ylabel('u(x,t)');
    title(sprintf('Exact | t = %s', t));
