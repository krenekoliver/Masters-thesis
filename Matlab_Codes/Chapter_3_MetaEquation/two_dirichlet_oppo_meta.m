...
u1 = sin(kappa1 * x_uniform) .* (x_uniform <= pi/2);
v1 = -kappa1 * cos(kappa1 * x_uniform) .* (x_uniform <= pi/2);

u2 = sin(kappa2 * x_uniform) .* (x_uniform >= 3*pi/2);
v2 = +kappa2 * cos(kappa2 * x_uniform) .* (x_uniform >= 3*pi/2);
...
u0_cheb = (sin(kappa1 * x_cheb) .* (x_cheb <= pi/2) + sin(kappa2 * x_cheb) .* (x_cheb >= 3*pi/2));

v0_cheb = (-kappa1 * cos(kappa1 * x_cheb) .* (x_cheb <= pi/2) +kappa2 * cos(kappa2 * x_cheb) .* (x_cheb >= 3*pi/2));
...
u0_fine = (sin(kappa1 * x_fine) .* (x_fine <= pi/2) + sin(kappa2 * x_fine) .* (x_fine >= 3*pi/2));
v0_fine = (-kappa1 * cos(kappa1 * x_fine) .* (x_fine <= pi/2) + kappa2 * cos(kappa2 * x_fine) .* (x_fine >= 3*pi/2));
...
% plot(x_cheb, u_lgcc_exact(plot_time),    'Color', [0 0.6 0.3], 'LineWidth',2.5, 'DisplayName','LGCC');
 plot(x_full, u_exact_plot(plot_time),    'k--', 'LineWidth',2.5, 'DisplayName','Exaktní');

xlabel('x');
ylabel('u(x,t)');
title(sprintf('FD | t = %s', formatThousands(plot_time)));
ax = gca;
...
print('-depsc2', '-r300', 'two_dirichlet_oppo_meta_FD.eps');

% Video
video_filename = fullfile('videos', 'two_dirichlet_oppo_meta.avi');
