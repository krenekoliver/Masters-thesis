...
u0 = (sin(kappa1 * x_uniform) + sin(kappa2 * x_uniform)) .* (x_uniform <= pi);
v0 = (-kappa1 * cos(kappa1 * x_uniform) - kappa2 * cos(kappa2 * x_uniform)) .* (x_uniform <= pi);
...
u0_cheb = (sin(kappa1 * x_cheb) + sin(kappa2 * x_cheb)) .* (x_cheb <= pi);
v0_cheb = (-kappa1 * cos(kappa1 * x_cheb) - kappa2 * cos(kappa2 * x_cheb)) .* (x_cheb <= pi);
...
u0_fine = (sin(kappa1 * x_fine) + sin(kappa2 * x_fine)) .* (x_fine <= pi);
v0_fine = (-kappa1 * cos(kappa1 * x_fine) - kappa2 * cos(kappa2 * x_fine)) .* (x_fine <= pi);
...
title(sprintf('Exact | t = %s', formatThousands(plot_time)));
ax = gca;
...
print('-depsc2', '-r300', 'two_dirichlet_same_meta_initial.eps');

%% ====================== VIDEO ======================
video_filename = fullfile('videos', 'two_dirichlet_same_meta.avi');
