...
dot0_sin = [0; 0];
C_sin = V_mat \ y0_sin;
B_sin = (V_mat \ dot0_sin) ./ omegas;

y0_cos = [0; 0];
dot0_cos = [-kappa; 0];
C_cos = V_mat \ y0_cos;
B_cos = (V_mat \ dot0_cos) ./ omegas;

u_sin_c1 = C_sin(1) * v1(1);
u_sin_c2 = C_sin(2) * v2(1);
u_cos_b1 = B_cos(1) * v1(1);
u_cos_b2 = B_cos(2) * v2(1);

u_exact_fun = @(t, x) (u_sin_c1 * cos(omega1*t) + u_sin_c2 * cos(omega2*t)) .* sin(kappa * x) + (u_cos_b1 * sin(omega1*t) + u_cos_b2 * sin(omega2*t)) .* cos(kappa * x);
...
print('-depsc2', '-r300', 'one_periodic_meta_FD.eps');

%% Video
video_filename = fullfile('videos', 'one_periodic_meta.avi');
